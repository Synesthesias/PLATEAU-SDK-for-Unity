...
namespace PlateauUnitySDK.Editor.FileConverter.GUITabs
{
    ...
    public class ConvertFileSelectorGUI
    {
        ...
        public void PrintConvertButton(IFileConverter converter)
        {
            ...
            using (PlateauEditorStyle.VerticalScope())
            {
                if (PlateauEditorStyle.MainButton("Convert")) ButtonConvertPushed(converter);
            }
        }
        ...
        {
            var path = EditorUtility.OpenFilePanel($"Select {fileExtension} File", this.sourceFilePath, fileExtension);
        ...
        {
            var result = converter.Convert(this.sourceFilePath, this.destinationFilePath);
