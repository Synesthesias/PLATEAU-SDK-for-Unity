                ...
                using (PlateauEditorStyle.VerticalScopeLevel1())
                {
                    ...
                    if (conf.loadPackage)
                    {
                        using (PlateauEditorStyle.VerticalScopeLevel1(1))
                        {
                            ...
                            EditorGUILayout.LabelField("テクスチャのインポート (オプション)", GUILayout.Width(400));
                            conf.includedTexture = (Texture2D)EditorGUILayout.ObjectField(conf.includedTexture, typeof(Texture2D), false);
