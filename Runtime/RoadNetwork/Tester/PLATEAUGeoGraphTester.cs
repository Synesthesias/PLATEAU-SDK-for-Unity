    ...
    public class PLATEAUGeoGraphTester : MonoBehaviour
    {
        ...
        public void OnDrawGizmos()
        {
            ...
            if ((lerpLineTest?.showLerpLineTest ?? false) && lerpLineTest.target)
            {
                ...
                DebugUtil.DrawArrow(GetVertex(p.indexA).Xay(), GetVertex(p.indexA + 1).Xay(), bodyColor: Color.red);
                DebugUtil.DrawArrow(GetVertex(p.indexA).Xay(), GetVertex(p.indexA - 1).Xay(), bodyColor: Color.blue);
