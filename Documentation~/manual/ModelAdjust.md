...
動的タイルを対象に設定した場合、処理完了後に対応するアセットバンドルが自動的に上書き更新されます。
...
`調整対象の種類`を`シーン内のオブジェクト`か`動的タイル`か選びます。  
...
##### 動的タイルを対象とする場合
- タイル選択ボタンから対象を選択します。  
- 「階層を表示」ボタンを押すことでタイル内の特定のゲームオブジェクトを対象と出来ます。
  - 階層の親にチェックが付いている場合、子もすべて対象とした扱いとなります。
- 「選択要素を含むタイルを追加」「選択中のn個を追加」ボタンは、シーン内で選択されているゲームオブジェクトまたはタイルを対象に加えます。
- 「パッケージ種から選択」ボタンでは種類を選ぶウィンドウが開きます。
...
`変換対象`ではシーン内のオブジェクトまたは動的タイルを選択できます。

#### シーン内のオブジェクトを対象とする場合

- PLATEAUInstancedCityModelが対象です。
- 土地起伏が含まれることが前提となります。

#### 動的タイルを対象とする場合

- PLATEAUTileManagerが対象です。
- 土地起伏タイルが含まれることが前提です。
- タイル選択ボタンで下図右のようなウィンドウが開きます。タイルを選択して決定ボタンを押します。

![動的タイル選択ウィンドウの例](../resources/manual/cityAdjust/DynamicTiles.png)

- タイル選択ウィンドウの操作方法について
  - `階層を表示`ボタンを押すとタイル内の各地物が展開されます。必要な地物だけを選択して追加できます。
