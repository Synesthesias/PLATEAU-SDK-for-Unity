...
動的タイルをゲームオブジェクトON/OFF機能の対象として指定できます。  
動的タイルを対象に設定した場合、処理完了後に対応するアセットバンドルが自動的に上書き更新され、変更結果が即座に反映されます。
...
動的タイルを分割・結合・マテリアル分け機能の対象として指定できます。  
動的タイルを対象に設定した場合、処理完了後に対応するアセットバンドルが自動的に上書き更新され、変更結果が即座に反映されます。
...
- `地形変換`にチェックを入れると地形モデルの変換(平滑化・テレインへの変換)が有効になり、更に詳細なオプションが表示されます。
  ...
  - `余白を端の高さに合わせる`にチェックを入れるとハイトマップの余白を一番端のピクセルで埋めます。チェックが外れていると余白部分の高さは０になります。余白とは、地形モデルが作成されていない箇所を指し、河川や海等が該当します。
...
動的タイルを分割／結合／マテリアル分けの対象として指定できます。 
動的タイルを扱う際には次のような操作が可能です。

![動的タイル選択ウィンドウの例](../resources/manual/cityAdjust/DynamicTiles.png)

 - シーン中で選択したオブジェクトから追加

 - タイル内の個別要素を指定
   - タイル選択ウィンドウではタイルが階層構造で表示され、`階層を表示`ボタンを選択することで子階層（タイル内の各地物）を展開します。必要な地物だけを選択して追加できます。
   - 親階層にチェックが入っている場合は、子階層もすべて選択扱いとなります。

### 動的タイルの表示制御

動的タイルの表示品質や描画挙動を制御するための機能です。
重要なエリアやランドマークを常に高品質で表示したい場合などに使用します。

### 常に高解像度で表示するタイル選択

「常に高解像度で表示するタイル選択」機能を使用することで、  
特定の動的タイルを常に最高解像度で固定表示できます。

![高解像度表示タイル選択の例](../resources/manual/cityAdjust/highquality.png)

例えば、重要なランドマークや注目エリアなど、低解像度にしたくないタイルを常に高解像度で表示したい場合 に有効です。

#### 選択できるタイル
- 選択可能なタイルは、最も高解像度である `tile_zoom_11` から始まるタイルのみです。
- 一覧には、プロジェクトに含まれる該当タイルが表示されます。
- 検索欄からタイル名で絞り込み検索が可能です。