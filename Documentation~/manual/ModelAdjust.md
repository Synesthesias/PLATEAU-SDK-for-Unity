...
![](../resources/manual/modelAdjust/DynamicTiles.png)

 - シーン中で選択したオブジェクトから追加

 - タイル内の個別要素を指定
   - タイル選択ウィンドウではタイルが階層構造で表示され、`階層を表示`ボタンを選択することで子階層（タイル内の各地物を展開できます。必要な地物だけを選択して追加することができます。
   - 親階層にチェックが入っている場合は、子階層もすべて選択されます。
