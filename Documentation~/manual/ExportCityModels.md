# 3D都市モデルのエクスポート
配置された3D都市モデルを、FBX、OBJ、glTFとしてエクスポートできます。  
このページではエクスポートの手順を説明します。

## 前提
都市モデルがインポートされていることが前提です。  
...
- `選択オブジェクト` からエクスポート対象を選択します。調整対象として「シーンに配置されたオブジェクト」または「動的タイル」を選択できます。
  - 「シーンに配置されたオブジェクト」が対象の場合
  ...
  - 「動的タイル」が対象の場合
    - 選択したPLATEAUTileManagerが保持する全タイルが対象となります。
- 出力形式を選択します。
