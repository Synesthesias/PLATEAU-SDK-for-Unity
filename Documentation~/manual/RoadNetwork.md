...
ただし、再度道路ネットワークを生成する際は元の道路を表示する必要があるため表示後に実行してください。※コメント
...
※この辺りに、ネットワーク生成の優先順位（歩道→車道の順番で生成するが、道路幅が狭く、入力したパラメータで歩道の生成ができない場合、車道のみを生成する?）に関する記載を追加してください。意味が伝われば文章フロー図でも構いません。
...
- `各レーンを個別編集`: 歩道と車道の各レーンについて、線を1つづつ編集します。

##### 全車道を一括編集
使い方：
- 編集モードで道路が選択されているとき、「全車道を一括編集」の開始ボタンを押します。  
- 道路の中心に、青い球体で挟まれた線が出現します。これが編集中の線です。
...
- 「終了」を押した時、またはEnterキーを押した時、スプラインに沿って道路が変形します。
- Escキーでキャンセルします。
...
- Rキーで緑色の線を回転させ、良ければEnterキーで決定します。やり直す場合はEscキーでキャンセルします。
- 交差点が生成されます。
