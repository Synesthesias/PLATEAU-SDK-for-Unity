...
- 生成パラメータ  
  - `車線幅（メートル）`：3D都市モデルの道路幅と車線幅を基準に車線が生成されます。車線数や形状に影響します。ただし、LOD3.1以上の道路で3D都市モデルから車道幅が分かる場合はモデルが優先されます。    
...
- エクスポートされるデータの仕様に関しては、[こちらのページ(道路ネットワークのエクスポートの仕様)](./RoadNetworkExported.md)をご参照ください。
