...
- ポリゴン数の低いLOD1の建物に、LOD2のテクスチャをTriplanarシェーダーで投影することでポリゴン数を下げます。
...
- テクスチャ解像度の1辺の大きさを何倍にするかを指定します。解像度を下げることで描画負荷を下げ軽量化できます。
  - ズームレベル11（カメラが近いときに読み込まれるタイル）、ズームレベル10（カメラがやや遠いときに読み込まれるタイル）についてそれぞれ設定できます。
  - ズームレベル9（カメラが遠いときに読み込まれるタイル）については、別の手法で描画負荷を下げるため設定項目はありません。詳しくは後述の「LOD2テクスチャ投影による建物モデルの軽量化」をご覧ください。
...
動的タイル機能では、建物データの軽量化のために、LOD1の建物形状に対し、LOD2の外観をTriplanarシェーダーで投影する方式を採用しています。

具体的には以下の処理を行っています。
- LOD2の3Dモデルを各方向からキャプチャ
  - 建物の外観を平面的な画像（テクスチャ）として生成します。
- LOD1の低ポリゴンモデルに対してTriplanarシェーダーで投影

この最適化により3Dモデルのポリゴン数を大幅に削減しつつ、LOD2に近い外観を維持できます。これによりポリゴン数を抑え、軽快に動作させることを意図しております。  
ただし、LOD2以上のテクスチャを含む建物をインポートしているときのみ動作します。  
Triplanarにより投影しているので、位置を変えると見た目がずれます。DynamicTileManagerや各タイルのPositionを(0,0,0)にしてご利用ください。

## 常に高解像度で表示するタイル選択

PLATEAUTileManagerのインスペクタから、常に高解像度で表示するタイルを選択できます。  
ここで指定されたタイルはカメラが離れても低解像度タイルではなく最も高解像度のタイルが利用されます。  
これにより重要なランドマークなどを遠くからでも強調できます。

![高解像度表示タイル選択の例](../resources/manual/cityAdjust/highquality.png)


### 選択できるタイル
- 選択可能なタイルは、最も高解像度である `tile_zoom_11` （ズームレベル11）から始まるタイルのみです。
- 検索欄からタイル名で絞り込み検索ができます。
- 「保存/反映」ボタンを押すことで反映されます。
